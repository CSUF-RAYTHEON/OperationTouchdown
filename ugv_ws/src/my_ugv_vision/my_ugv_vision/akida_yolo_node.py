...
class AkidaYoloNode(Node):
    ...
    def _try_load_akida(self, model_path: str) -> None:
        ...
        if dev_list:
            ...
            self.get_logger().info(
                f"AKD1000 hardware found: {device}. Mapping model...")
        else:
            self.get_logger().warn(
                "No AKD1000 hardware visible; falling back to a virtual device. "
                "Check 'akida devices' and the PCIe driver. Inference will be "
                "very slow.")
            try:
                from akida import AKD1000  # type: ignore
                device = AKD1000()
                self.device_kind = 'virtual'
            except Exception as exc:
                self.get_logger().error(
                    f"Could not instantiate virtual AKD1000: {exc}")
                self.model = None
                return
        ...
        try:
            ...
            self.get_logger().error(f"model.map() failed: {exc}")
            self.model = None
            return
    ...
    def _publish_diagnostics(self) -> None:
        ...
        elif self.device_kind == 'virtual':
            status.level = DiagnosticStatus.WARN
            status.message = 'running on virtual Akida device'
        else:
