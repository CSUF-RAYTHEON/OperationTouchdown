...
class MapMode(Enum):
    ...
    """
    AllNps = 1
    """ Maximize HW resources (number of NPs used) with minimum HW passes."""
    HwPr = 2
    """ Maximize HW resources (number of NPs used) with maximum HW passes.
    This mode provides the potential for higher-performances"""
    Minimal = 3
    """ Minimize HW resources"""
...
def _map_search(model, device, hw_only, min_pass, constraints):
    ...
    if any(layer.splittable for layer in model.layers):
        # Obtains the reference mapped model, using the minimal hardware resources
        try:
