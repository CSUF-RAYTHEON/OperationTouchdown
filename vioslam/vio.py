...
MAX_CORNERS = 300 # Max features tracked. Increasing makes tracking robust but eats CPU. Decreasing saves CPU but risks losing ground lock on smooth floors.
QUALITY_LEVEL = 0.01 # Minimum sharpness score for a corner. Increasing forces the math to only use high-contrast edges. Decreasing allows tracking in dark/blurry areas but adds noise to the math.
MIN_DISTANCE = 10 # Minimum pixel gap between points. Increasing forces points to spread evenly across the camera view (better rotation math). Decreasing bunches points together, risking tracking a single noisy cluster.
LK_WIN_SIZE = (21, 21) # Pixel box size to search for the point in the next frame. Increasing allows drone to fly much faster without losing points. Decreasing saves CPU but drone loses tracking if it jerks suddenly.
LK_MAX_LEVEL = 3 # Number of times the image is shrunk for macro-tracking. Increasing allows surviving massive frame-to-frame pixel jumps (high speed). Decreasing saves CPU but limits top physical speed.
MIN_PNP_POINTS = 10 # Minimum points required to calculate 3D movement. Increasing makes position math rock-solid and prevents wild jumps. Decreasing prevents "tracking lost" crashes but makes the flight path highly inaccurate.
...
REDETECT_EVERY = 7 # Redetect features every N frames. Lowering improves probability of not using stale or blurry floor data during a rapid movement.
SOFT_CORR_ALPHA = 0.01 # Percentage of SLAM vector applied at once. Increasing snaps the drone to the map instantly (causes violent flight controller jerks). Decreasing smooths out the flight path but takes longer to eliminate drift.
...
def vio(gray_frame_mutex, depth_frame_mutex, attitude_mutex, position_mutex, slam_trigger_mutex):
    set_core_and_priority(4, -20) # Core 4, Max Priority
