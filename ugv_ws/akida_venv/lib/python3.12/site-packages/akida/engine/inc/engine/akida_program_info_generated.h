...
struct ProgramInfo FLATBUFFERS_FINAL_CLASS : private flatbuffers::Table {
  ...
  bool anomaly_detection() const {
    return GetField<uint8_t>(VT_ANOMALY_DETECTION, 0) != 0;
  }
  bool Verify(flatbuffers::Verifier &verifier) const {
    return VerifyTableStart(verifier) &&
           ...
           VerifyField<uint8_t>(verifier, VT_ANOMALY_DETECTION, 1) &&
           verifier.EndTable();
  }
};
...
struct ProgramInfoBuilder {
  ...
  void add_anomaly_detection(bool anomaly_detection) {
    fbb_.AddElement<uint8_t>(ProgramInfo::VT_ANOMALY_DETECTION, static_cast<uint8_t>(anomaly_detection), 0);
  }
...
inline flatbuffers::Offset<ProgramInfo> CreateProgramInfo(
  ...
  builder_.add_anomaly_detection(anomaly_detection);
