...
static uint8_t target_to_uid(const Target& target) {
  switch (target) {
    ...
    case Target::LookUpTable:
      return HDR_UID_LUT;
    case Target::PicoRegisters:
      return HDR_UID_PICO_REGS;
    case Target::PicoFsram:
      return HDR_UID_PICO_FSRAM;
    case Target::PicoTsram:
      return HDR_UID_PICO_TSRAM;
    default:
      break;
  }
...
void format_config_header(uint32_t* header, const struct hw::Ident& np,
  ...
  // check if the target is Pico
  const bool pico_target =
      (target == Target::PicoRegisters || target == Target::PicoFsram ||
       target == Target::PicoTsram);
  bool hrc_en = (np == hw::HRC_IDENT) && !pico_target;
  ...
  set_field(&header[HDR_WORD1], HDR_NP_DST,
            (hrc_en || pico_target) ? 0x0 : 1u << np.id);
