 ...
 *
 * @param driver: the HardwareDriver object used by the current device
 * @param dma: dma::Input object on which the last job id will be retrieved
 *
 * @return ID of last processed job read from dma
 */
uint16_t get_last_job_id_processed(HardwareDriver* driver, const Input& dma);
 ...
 *
 * @param driver: the HardwareDriver object used by the current device
 * @param dma: dma::Input object to check if clock counter is enabled
 *
 * @return true if clock counter is enabled, false if it is disabled
 */
bool is_buffer_timer_enabled(const HardwareDriver& driver, const Input& dma);
 ...
 *
 * @param driver: the HardwareDriver object used by the current device
 * @param dma: dma::Input object to toggle pipeline mode
 * @param enabled: true if pipeline must enabled, false if it must be disabled
 */
void toggle_pipeline(HardwareDriver* driver, const Input& dma, bool enabled);

/**
 * @brief Configure input dma for multipass mode
 *
 * @param driver: the HardwareDriver object used by the current device
 * @param dma: dma::Input object to be configured
 ...
 */
void prepare_engine_multi_pass(HardwareDriver* driver, const Input& dma,
 ...
 *
 * @param driver: the HardwareDriver object used by the current device
 * @param dma: dma::Input object ton configure
 * @param clear_size: Size (in bytes) that will be zeroed at each output. It
 * will be 32 bits aligned
 */
void set_output_buffer_clear(HardwareDriver* driver, const Input& dma,
                             uint32_t clear_size);
 ...
 * across NPs so there will be a single extra descriptor in a program
 *
 * @param driver: the HardwareDriver object used by the current device
