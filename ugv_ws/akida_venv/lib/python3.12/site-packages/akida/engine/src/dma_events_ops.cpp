...
TensorUniquePtr dma_events_read_outputs(HardwareDriver* driver,
                                        ...
                                        const ProgramInfo& program_info) {
  const bool is_pico =
      program_info.device_version().get_ip_version() == IpVersion::pico;
  const size_t type_size = program_info.output_bits() / 8U;
  const auto output_dimensions =
      is_pico
          ? conversion::get_aligned_shape(program_info.output_dims(), type_size)
          : program_info.output_dims();
  ...
  switch (program_info.outputs_type()) {
    ...
    case fb::IoType_dense: {
      ...
      if (is_pico) {
        dense = conversion::unaligned_channels_from_32b(
            std::move(dense), program_info.output_dims()[2]);
      }
      return dense;
    }
    default:
