...
static Descriptor format_event_desc(uint32_t job_id, uint32_t input_addr,
                                    uint32_t output_addr, uint32_t buf_sz,
  ...
  set_field(&descriptor[event::DESC_WORD2], event::DESC_INPUT_ADDR, input_addr);
  set_field(&descriptor[event::DESC_WORD3], event::DESC_DATA_BUF_SZ, buf_sz);
  set_field(&descriptor[event::DESC_WORD4], event::DESC_OUTPUT_ADDR,
            output_addr);
...
static Descriptor format_hrc_desc(uint32_t job_id, uint32_t input_addr,
                                  uint32_t output_addr, uint32_t row_bytesize,
                                  uint32_t height, uint32_t next_row_offset,
                                  uint32_t window_row_bytesize,
...
static Descriptor dma_dense_descriptor(uint32_t job_id, uint32_t input_addr,
  ...
  // generate descriptor
  return format_hrc_desc(job_id, input_addr, output_addr, row_byte_size,
                         col_height, next_row_offset, window_row_byte_size,
                         window_h, window_overlap_bytesize, y_offset, x_offset,
                         learning_class);
}
...
Descriptor format_input_desc(const dma::Input& input_dma, uint32_t job_id,
                             ...
                             uint32_t window_h) {
  if (typeid(input_dma) == typeid(dma::Hrc)) {
    return dma_dense_descriptor(job_id, input_addr, output_addr, learning_class,
                                input_shape, window_w, window_h);
  }
  return format_event_desc(job_id, input_addr, output_addr, buffer_size,
                           learning_class);
}
